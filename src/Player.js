...
class Player extends Component {
  constructor(props) {
    ...
    this.socket.on('chose a domain', function (data) {
      this.setState({ time: data.time, categories: data.message, page: 'category' })
    }.bind(this));

    this.socket.on('answer the question', function (data) {
  ...
  sendCategoryChosenByPlayer(category) {
    this.socket.emit('catagory picked', { category: category });
    this.setState({ page: 'blank' });
  }

  sendAnswerGivenByPlayer(answer) {
    this.socket.emit('given answer', { answer: answer });
    this.setState({ page: 'blank' });
  }

  sendAnswerVotedByPlayer(answer) {
    this.socket.emit('vote', { answer: answer });
    this.setState({ page: 'blank' });
  }
